...
class TripAdvisorDiscovery:
    ...
    def __init__(
        self,
        seeds_dir: str = "seeds",
        output_file: str = "outputs/entity_registry.json",
        headless: bool = False,          # Nên để False
    ...
    async def start_browser(self):
        ...
        # Dùng Chrome thật + headed
        self.browser = await playwright.chromium.launch(
            headless=self.headless,
            channel="chrome",  # Quan trọng: dùng Chrome đã cài trên máy
            args=[
    ...
    async def search_province(self, page: Page, province: str) -> Optional[str]:
        """
        1. Vào tripadvisor.com
        2. Gõ tên tỉnh
        3. Lấy geo code từ typeahead → tạo URL Attractions
        """
        ...
        # Ô search chính
        search_input = page.locator('input[placeholder="Places to go, things to do, hotels..."]')
        ...
        await self.human_delay(800, 1400)

        # Đợi typeahead xuất hiện
        typeahead = page.locator('div#typeahead_results')
        try:
        ...
        # Lấy tất cả option trong typeahead
        ...
        for i in range(count):
            ...
            # Chỉ lấy kết quả là địa điểm (Tourism-gXXXX)
            if href and "/Tourism-g" in href and province.lower() in text:
                # Ví dụ: /Tourism-g469421-Vinh_Long_Vinh_Long_Province_Mekong_Delta-Vacations.html
                match = re.search(r"/Tourism-(g\d+)-(.+?)-Vacations\.html", href)
                if match:
                    geo_code = match.group(1)          # g469421
                    slug = match.group(2)              # Vinh_Long_Vinh_Long_Province_Mekong_Delta
        ...
        # Tạo URL Attractions
        attractions_url = f"https://www.tripadvisor.com/Attractions-{geo_code}-Activities-oa0-{slug}.html"
    ...
    async def scrape_attractions(self, page: Page, attractions_url: str, province: str) -> List[EntityCandidate]:
        """
        Vào trang Attractions → lấy danh sách entity + phân trang bằng nút Next
        """
        ...
        while True:
            ...
            # Tìm nút Next page
            next_btn = page.locator('a[data-smoke-attr="pagination-next-arrow"], a[aria-label="Next page"]')
    ...
    async def run(self):
        ...
        print(f"Bắt đầu discovery {len(provinces)} tỉnh trên TripAdvisor\n")
        ...
        try:
            ...
            await stealth_async(page)          # ← áp dụng stealth lên page
...
if __name__ == "__main__":
    ...
    discovery = TripAdvisorDiscovery(headless=False, max_provinces=2)
